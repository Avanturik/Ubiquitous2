...
namespace WpfAnimatedGif
{
    ...
    public static class ImageBehavior
    {
        ...
        public static object GetCompressedSource(Image obj)
        {
            return (object)obj.GetValue(CompressedSourceProperty);
        }
        ...
        /// <param name="value">true if the animation should start immediately when loaded. Otherwise, false.</param>
        /// <remarks>The default value is true.</remarks>
        public static void SetCompressedSource(Image obj, object value)
        {
            obj.SetValue(CompressedSourceProperty, value);
        }

        /// <summary>
        /// Identifies the <c>AutoStart</c> attached property.
        /// </summary>
        public static readonly DependencyProperty CompressedSourceProperty =
            DependencyProperty.RegisterAttached("CompressedSource", typeof(object), typeof(ImageBehavior), new PropertyMetadata(null, (obj, e) => {
                ...
                if( e.NewValue is CompressedBitmapSource )
                {
                    image.Source = (e.NewValue as CompressedBitmapSource);
                }
                else if( e.NewValue is BitmapFrame )
                {
                    image.Source = e.NewValue as BitmapFrame;
                }
            ...
            if (source != null && shouldAnimate && !isLoadingDeferred)
            {
                ...
                if (animation != null)
                {
                    if (animation.KeyFrames.Count > 0)
                    {
                        ...
                        TryTwice(() => imageControl.Source = animation.KeyFrames[0].Value as CompressedBitmapSource);
                    }
                    else
                    {
                        imageControl.Source = source;
        ...
        public static byte[] ConvertToByteArray(this BitmapSource renderTarget)
        {
            if (renderTarget == null || renderTarget.PixelHeight == 0 || renderTarget.PixelWidth == 0)
                return null;

            int stride = renderTarget.PixelWidth * 4;
